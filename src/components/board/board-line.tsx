...
export default function BoardLine({size, gameResult}: BoardLineProps): ReactElement {
  const diagonalHeight = Math.sqrt(Math.pow(size,2) + Math.sqrt(Math.pow(size,2)));
    ...
    <>
      {
        ...
        <Animated.View style={[style.line, style.vLine, 
        {
      ...
      { 
        ...
        <Animated.View style={[style.line, style.hLine, 
        {
      ...
      }
      {
        ...
        <Animated.View style={[style.line, style.dLine, 
        {
          height: animationRef.current.interpolate({
            inputRange: [0,1],
